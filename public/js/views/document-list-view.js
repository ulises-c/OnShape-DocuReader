...
import {
  ...
  renderDocumentRows,
} from "./helpers/pagination-renderer.js";
...
export class DocumentListView extends BaseView {
    ...
    qsa(".pagination-btn", this.container).forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (action === "first") targetPage = 1;
          ...
          targetPage = Math.max(1, pagination.currentPage - 1);
        else if (action === "next")
          targetPage = Math.min(
  ...
  _notifySelectionChanged() {
    const checked = qsa(".doc-checkbox:checked", this.container).map(
      (cb) => cb.value
    );
  ...
  captureState() {
    try {
      const scrollContainer =
        this.container?.closest("[data-scroll-preserve]") || null;
      const selectedIds = qsa(".doc-checkbox:checked", this.container).map(
        (cb) => cb.value
      );
      ...
      return {
        scroll: {
          ...
          containerKey:
            scrollContainer?.getAttribute?.("data-scroll-key") || null,
        },
        selectedIds,
  ...
  restoreState(state) {
    ...
    try {
      ...
      if (state.pagination && this.controller) {
        ...
        if (
          currentPage !== this.controller.pagination.currentPage ||
