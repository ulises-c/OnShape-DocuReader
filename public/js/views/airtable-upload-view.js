...
export class AirtableUploadView extends BaseView {
    ...
    const html = `
      <div class="airtable-upload-panel">
        ...
        <div class="upload-instructions">
          ...
          <code>{part_number}_{name}.png</code>
  ...
  async _startUpload() {
    ...
    try {
      ...
      console.log(
        "[AirtableUploadView] Stored results for download:",
    ...
    tableBody.innerHTML = results
      .map((r, index) => {
        ...
        <tr class="${statusClass}">
          ...
          <td class="filename-cell" title="${escapeHtml(
            r.filename || ""
          )}">${escapeHtml(r.filename || "—")}</td>
          ...
          <td class="details-cell">${escapeHtml(
            r.error || r.recordId || "—"
          )}</td>
        </tr>
      `;
  ...
  _downloadReport(format) {
    console.log(
      "[AirtableUploadView] Download report requested:",
      format,
      "hasResults:",
      !!this._lastResults
    );
    ...
    console.log(
      "[AirtableUploadView] Generating",
      format,
      "report with",
      this._lastResults.results?.length || 0,
      "items"
    );
    ...
    if (format === "json") {
      this._downloadJson(
        this._lastResults,
        `airtable-${modeLabel}-report-${timestamp}.json`
      );
    } else if (format === "csv") {
  ...
  _resultsToCSV(results) {
    ...
    const csvLines = [
      ...
      `# Total: ${results.summary?.total || 0}, Uploaded: ${
        results.summary?.uploaded || 0
      }, Skipped: ${results.summary?.skipped || 0}, No Match: ${
        results.summary?.noMatch || 0
