...
async function fetchThumbnailBlob(thumbnailUrl) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[FullExtract] Thumbnail fetch failed (${response.status}): ${thumbnailUrl}`);
      return null;
    }
...
async function loadJSZip() {
  try {
    ...
    console.warn('[FullExtract] Local JSZip not found, trying CDN...');
    try {
...
export async function fullAssemblyExtract(options) {
  ...
  try {
    ...
    if (!csvContent) {
      console.warn('[FullExtract] CSV conversion returned empty, continuing with JSON only');
    }
