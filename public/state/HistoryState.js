...
class HistoryState {
  ...
  constructor(stateManager = null, options = {}) {
    ...
    this.strategies = options.strategies && typeof options.strategies === 'object'
      ? { ...options.strategies }
      : {};
  }
  ...
  _restoreViewState(viewType, snapshot) {
    ...
    try {
      if (typeof document !== 'undefined' && snapshot && typeof snapshot === 'object') {
    ...
    for (const selector of this.scrollSelectors) {
      try {
        ...
        for (const el of nodes) {
          if (!el) continue;
          const key = el.getAttribute('data-scroll-key') || el.id || this._nodeKey(el);
    ...
    if (Array.isArray(scroll.containers)) {
      for (const c of scroll.containers) {
        try {
          let el = null;
          if (c.key) {
            el = document.querySelector(`[data-scroll-key="${CSS.escape(c.key)}"]`) ||
